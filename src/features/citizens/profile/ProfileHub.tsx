...
export default function ProfileHub({ reports, score, now }: { reports: CitizenReport[]; score: CivicScore; now: number }) {
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-6 sm:gap-8 xl:grid-cols-12">
      <div id="sesizari" className="scroll-mt-20 space-y-4 xl:col-span-7">
        <div className={`space-y-3 rounded-xl border p-4 shadow-sm ${panel}`}>
          ...
          <div className="grid grid-cols-1 gap-2.5 pt-1 sm:grid-cols-12">
            <label className="relative sm:col-span-4">
              ...
              <Icon node={Search} className="absolute left-3 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-slate-400" />
              <input
                type="search"
                value={query}
            ...
            <label className="sm:col-span-4">
              ...
              <select value={category} onChange={(e) => setCategory(e.target.value)} className={`px-2.5 py-2 ${field}`}>
        ...
        {reports.length === 0 ? (
          ...
          <p className={`rounded-xl border p-6 text-center text-xs ${panel} ${muted}`}>Nicio sesizare nu se potrivește filtrelor.</p>
        ) : (
          shown.map((r) => (
      ...
      <div className="space-y-4 xl:col-span-5">
        {selected && <ReportInspector report={selected} reports={reports} selection={selection} onPick={pickFromMap} now={now} />}
