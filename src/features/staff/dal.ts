...
import { currentSession, type Role, type SessionResult } from "@/features/staff/session";
...
export async function requireStaffPage(path: string, opts: Options = {}): Promise<SessionResult> {
  ...
  if (result.user.forcePasswordReset && !opts.allowPasswordReset) redirect("/account/password");
