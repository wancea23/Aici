...
export default function StaffHeader({ user, subtitle }: { user: StaffUser; subtitle: string }) {
  return (
    <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
      <div>
        <Link href="/panou" className="text-2xl font-semibold tracking-tight text-brand-700 transition-colors hover:text-brand-800">
          Aici
        </Link>
      ...
      <nav className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-slate-500">
        ...
        <Link href="/cont" title={user.email} className="transition-colors hover:text-slate-800">
          Contul meu
        </Link>
