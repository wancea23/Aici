...
import { answerDeadline, deadlineText } from "../../src/features/reports/deadline";
import { formatDay } from "../../src/ui/format";
import { statusInput } from "../../src/features/reports/validation";
...
test("the status email points to the account and carries no message", async () => {
  const { statusMail } = await import("../../src/server/mail");
  ...
  assert.ok(mail.text.includes("https://aici.example/profile"));
